...
Antes, la traducción ES->JA para VOICEVOX vivía en ``miku/voz/salida/tts.py``.
    ...
    try:
        ...
        if cache is not None:
            if len(cache) >= _CACHE_MAX:
                # Descarta la entrada más vieja (los dict conservan el orden de
                # inserción): sin tope, cada frase única del LLM crecía la cache
                # para siempre.
