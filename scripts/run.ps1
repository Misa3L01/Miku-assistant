...
#
# El modo (voz / texto) se elige dentro de main.py con la ventanita al arrancar
# (o por consola si no hay PyQt5). El modo texto siempre habla.
# Para saltarte VOICEVOX usá:  .\run.ps1 -SinVoicevox
# ============================================================================
...
# La raiz del proyecto es la carpeta de arriba de scripts\.
$Raiz = if ($PSScriptRoot) { Split-Path -Parent $PSScriptRoot } else { Get-Location }
Set-Location $Raiz
...
# El marcador guarda el hash de requirements.txt: si el archivo cambió desde la
# última instalación, se vuelven a instalar las dependencias.
$FlagInstalado = Join-Path $Venv ".miku_deps_instalado"
$HashReq = (Get-FileHash "$Raiz\requirements.txt" -Algorithm SHA256).Hash
$HashPrevio = ""
if (Test-Path $FlagInstalado) { $HashPrevio = ((Get-Content $FlagInstalado -Raw) + "").Trim() }
if (($HashPrevio -ne $HashReq) -or $Reinstalar) {
    ...
    Set-Content -Path $FlagInstalado -Value $HashReq
}
