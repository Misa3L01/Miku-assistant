...
## 📅 Última actualización: Sesión 10 (todas las fases de la reestructuración, A a I, — ver `docs/plan_reestructuracion.md`)
...
- **Pendiente:** revisión final y merge de `reestructuracion` a `main` (lo decide el dueño); ideas: wake word local, recordatorios recurrentes, TIDAL probado con cuenta real.
...
- **Fase F (proveedores):** `BrainGroq.endpoint()` resuelve URL/clave/modelo/tools (Groq o `LLM_BASE_URL`); `voz/entrada/transcriptores.py` (`Transcriptor`, `WhisperGroq`, `WhisperLocal`, `crear_transcriptor`; `SpeechToText._llamar_whisper` delega y solo filtra alucinaciones); `voz/salida/motores.py` (`MotorComando`: comando externo sin shell, `{salida}`/`{texto}`, stdin UTF-8 con `PYTHONIOENCODING`; `nombre_de_idioma`) + `TextoAVoz._motor_elegido/_preparar_con_comando/_mostrar_subtitulo` (los artefactos de audio llevan `idioma`; `auto` = subtítulo solo si el idioma hablado no es español). Opciones: `LLM_*`, `STT_PROVEEDOR`, `STT_MODELO_LOCAL`, `TTS_MOTOR`, `TTS_IDIOMA`, `TTS_COMANDO`, `SUBTITULOS`. Los proveedores locales de LLM/STT no se probaron con modelos reales. Tests: `tests/test_proveedores.py`.
- **TIDAL por CDP (post-Fase F):** se comprobó con la cuenta real que `tidal://track/<id>` solo navega a la ficha. `multimedia/tidal_control.py` (`ControlTidal`) maneja TIDAL de escritorio por su puerto de depuración (`TIDAL_PUERTO_CONTROL`, 9223): `asegurar()` (reinicia/lanza `app-x.y.z/TIDAL.exe --remote-debugging-port`, con `ELECTRON_RUN_AS_NODE` quitado del entorno), `reproducir()` (`Page.navigate` a `desktop.tidal.com/track/<id>` y clic en `[data-test=play-button]`; si ya suena/está en pausa no navega), `boton()`, `ahora()`. El websocket necesita `suppress_origin=True` (Chromium rechaza CDP con cabecera Origin; también corregido en Brave). SMTC ahora prefiere la sesión de TIDAL. `python -m miku.ajustes validar` avisa si falta una librería opcional de lo configurado (Telegram, Discord, tidalapi, websocket-client). Descubierto: el bot de Telegram no respondía porque `python-telegram-bot` no estaba instalado en el venv. Tests: 446.
- **Tecla de invocación y aleatorio TIDAL:** `servicios/tecla.py` (detección con `keyboard.hook`, `sc:NN` para teclas sin nombre, `python -m miku.servicios.tecla` de diagnóstico); `Asistente.tecla_invocacion/elegir_tecla` y opción `TECLA_INVOCAR`; F22 sigue manejado por el atajo de Windows cuando está activo, cualquier otra tecla siempre dentro de Miku. TIDAL: `playlist_propia`/`elegir_playlist` (tus playlists por nombre, raíz de 4 letras), `ControlTidal.aleatorio()` (interruptor `aria-checked` del reproductor) y botón `shuffle-all` de las fichas de álbum/playlist/artista; un artista queda en aleatorio por defecto. **fastembed:** fallaba con "DLL load failed" porque PyQt5 cargaba antes que onnxruntime; `miku/app.py` precarga onnxruntime antes de importar Qt. Tests: 469.
- **Tecla F13 (reemplaza a F22):** `TECLA_INVOCAR` por defecto pasó a `f13`; `arranque.py` generalizado (`atajo_activo/activar_atajo(ruta, tecla)/desactivar_atajo/atajo_valido`, un acceso directo admite F1-F24 con modificadores; el hotkey interno se registra solo si el atajo de Windows no lo cubre; `elegir_tecla` rehace o quita el acceso directo). Se borró el acceso directo con F22 y se creó uno con F13. Insert->F13 a nivel sistema con `scripts/remapear_insert_a_f13.ps1` (Scancode Map `00..02 00 00 00 64 00 52 E0 00..`; requiere admin y reiniciar; `-Restaurar` lo revierte). Tests: 481.
- **Ronda de mejoras (feedback de uso):** (1) saludo corto al arrancar + regla proactiva `BriefingAlVolver` (`plataforma/inactividad.py` con `GetLastInputInfo`); (2) `ver_pantalla`: Gemini `gemini-2.0-flash` retirado (404) y luego sin créditos (402) → alias `gemini-flash-latest` + respaldo Groq `qwen/qwen3.8-27b` (ve imágenes) + parámetro `monitor`; (3) `listar_ventanas` filtra ventanas "cloaked" (DwmGetWindowAttribute 14), overlays (NOACTIVATE/TRANSPARENT), TOOLWINDOW y owned; nueva tool `listar_pestanas` por CDP; (4) `JUEGOS_LANZADOR` (`plugins/sistema/lanzadores.py`: lanzador → espera proceso → `ShellExecute runas` del juego); (5) `utiles.aperturas`: `Ventanas._hwnd_de` espera hasta 12 s la ventana de algo recién abierto; prompt con regla de cadenas; (6) `social/telegram_envio.py` (API HTTP de Telegram, sin python-telegram-bot; a contactos = peligrosa); (7) `volumen_tidal` (Ctrl+flechas por CDP, 10 puntos/pulsación; nivel leído de `%APPDATA%\\TIDAL\\logs\\player.log`). Tests: 532.
- **Comedor (SIU-Toba):** `plataforma/cdp.py` (`Navegador`: Brave aparte con `--user-data-dir=data/navegador_miku` **resuelto a ruta absoluta** —con ruta relativa Brave no abría el puerto— y `Pagina`: evaluar/esperar/escribir/clic/captura por CDP con `suppress_origin`); `plugins/productividad/comedor.py`: login (`#ef_form_103000002_datosusuario/clave`, `#form_103000002_datos_ingresar`), ir a `.../comedorfi/1.0/aplicacion.php?tm=1&tcm=central&ai=comedorfi||103000019`, `_JS_INSTANTANEA` (elementos con `data-miku-i`, tablas, texto), `decidir()` pura (fecha de mañana en varias formas; "anular" en la fila = ya inscripto; botón deshabilitado o aviso de paro = no habilitado; nada reconocible = `desconocido`, nunca inscribe a ciegas), verificación releyendo la página tras el clic, contraseña en `keyring` (Credential Manager), estado en `data/comedor_estado.json`; regla `ComedorDiario` (aviso o `COMEDOR_AUTO`, lunes a jueves, no en juego ni ausente); la macro `comedor` delega en el plugin. **Pendiente:** ajustar `decidir()` con una instantánea real (`comedor explorar`); todo lo demás se probó con un sitio simulado y el login real hasta el formulario. Tests: 565.
- **Portapapeles inteligente:** `plataforma/portapapeles.py` (leer/escribir/tipo con reintentos; el traductor lo usa), `cerebro/complemento.py` (`pedir_texto`: una consulta al LLM sin tools, respeta `LLM_BASE_URL`, quita `<think>`), `plugins/productividad/portapapeles_inteligente.py` (traducir/corregir/reescribir reemplazan y guardan el original para `deshacer`; resumir/explicar/leer se dicen). Probado en vivo con Groq y el portapapeles real (restaurado). config_local.py: bloque con los pasos del comedor y `COMEDOR_USUARIO` (línea 24). WhatsApp queda para después: un solo número (el principal), contacto por defecto = segundo número; vía WhatsApp Web en un perfil propio con QR una vez. Tests: 580.
- **Comedor, lectura real:** con el volcado de la página (`data/comedor_exploracion.json`, `tests/fixtures/comedor_pagina_real.json` anonimizado) se reescribió `decidir()`: tabla `Tipo de Comida | Fecha comida | Descripcion | ... | Habilitado desde | Habilitado hasta | Cupo | Inscripto? | Ya Asistió`, botón `cuadro_103000076_cuadro{N}_seleccion` (N = fila), habilitación `dd/mm/aaaa HH:MM` del día anterior (14:00), `COMEDOR_TIPOS`. Corrige un bug real del primer diseño: el encabezado "Inscripto?" se leía como "ya inscripto" cuando no había botón. Una página sin la tabla (login, sesión vencida) ya no se interpreta como "no hay comida". Sin probar aún: el estado *después* de inscribirse (se vuelve a leer la página; el botón desaparece o la columna Inscripto? se marca) y la contraseña (falta `guardar-clave`). Tests: 596.
- **Comedor, login:** el fallo real era `COMEDOR_USUARIO = "'<dni>"` (apóstrofe pegado, error típico al copiar de una planilla): la página respondía "Usuario no es válido". Ahora `limpiar_usuario` quita comillas sueltas, `usuario_valido` exige `^[A-Za-z0-9_]+$` (lo que pide el formulario) y el mensaje de la página se incluye en `comedor.login_fallo`. `guardar-clave` pide la contraseña dos veces y confirma cuántos caracteres guardó (nunca la muestra). Prueba en seco real con la cuenta: login OK y `listo_para_inscribir` (elemento 35). Tests: 613.
- **Comedor, doble confirmación:** el paso 2 real es `¿Inscribirse? [form_103000077_formulario_inscribirse] [form_103000077_formulario_cancelar]`. El primer diseño recargaba la página tras el primer clic y **abandonaba** la confirmación pendiente; ahora `indice_de_confirmacion()` (contexto con "Cancelar" y "?", nunca el Cancelar) aprieta el segundo botón antes de releer. `data/comedor/ultima_lectura.json` guarda la última lectura (para ver cómo queda la fila ya inscripta). `Navegador.minimizar()` por CDP (`--start-minimized` no se respeta) y `COMEDOR_VENTANA`. Tests: 622.
- **Comedor, funcionando:** primera inscripción real OK (15:29). Estado real ya inscripto: columna `Inscripto?` = `SI`, `Ya Asistió` = `NO` y el botón `Inscribirse` desaparece (fixture `comedor_inscripto_real.json`). Reintentos automáticos (3/día, 30 min) y sin reintentar errores de configuración; los reintentos no repiten "todavía no hay comida". Tests: 632.
- **WhatsApp:** `plugins/social/whatsapp.py`. Brave aparte (`data/navegador_whatsapp`, puerto 9225, `--user-agent` de Chrome porque en headless el UA "HeadlessChrome" hace que WhatsApp Web se niegue a abrir; ventana "minimizada" por defecto). Estados de la página por JS (`invalido` > `chat` > `lista` > `qr`); verificado con la página real sin sesión (detecta `qr`). Enviar: `web.whatsapp.com/send?phone=<549...>`, `Input.insertText` + Enter, y confirma contando `#main [data-id^="true_"]`; archivos: `paste` con `DataTransfer` (base64 en trozos) + botón enviar de la vista previa. **Sin probar con sesión real** (falta escanear el QR). Números: `normalizar_numero` (`+54 3751 123456` -> `5493751123456`). Lo de enviar a terceros es tool peligrosa con confirmación.
