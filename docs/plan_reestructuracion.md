...
| I. Plugins y cerebro | ✅ hecha (con lo que quedó fuera abajo): traductor a cualquier idioma solo-portapapeles con perfiles por juego; `reproducir_en_tidal`/`conectar_tidal` (tidalapi opcional); snapshot de modos persistente + `volver_resolucion_nativa`; `listar_macros` corto; tools de memoria (`guardar/olvidar/listar_recuerdos`, `olvidar_conversacion`); historial de charla en el parser; enrutador de tools (`cerebro/enrutador.py`, corpus de 84 frases en tests); `cerrar_pestana` por título y `buscar_en_pestana_actual` real; recordatorios persistentes. **No hecho:** recordatorios recurrentes, SDK de Everything / ranking por recencia (ideas) |
| F. Proveedores intercambiables | ✅ hecha: LLM compatible con OpenAI (Groq por defecto, Ollama/LM Studio/otros con `LLM_BASE_URL`, tools opcionales); STT con interfaz `Transcriptor` (`WhisperGroq`, `WhisperLocal` con faster-whisper); TTS con `TTS_MOTOR` (voicevox / sistema / comando externo para voces propias), `TTS_IDIOMA` (traduce si no es español) y `SUBTITULOS = auto/siempre/nunca`. **No hecho:** wake word local (openWakeWord/Porcupine), motores TTS nativos (Piper/XTTS embebidos: se usan por comando) |
| E–I | todas hechas; queda la revisión final y el merge a `main` |
