...
class DiscordControl(Plugin):
    ...
    def __init__(self) -> None:
        ...
        self._listo = threading.Event()      # se setea cuando el bot conecta
        self._ultimo_error: str = ""         # último error de conexión (legible)
        self._activo = False                 # True si el bot está corriendo
        self._token = ""
    ...
    def initialize(self, event_bus: Any = None) -> None:
        ...
        # Creamos el hilo que corre el event loop del bot.
        self._thread = threading.Thread(
            target=self._correr_loop, name="discord-bot", daemon=True)
        self._thread.start()

        # Esperamos un rato a que conecte (no bloqueamos más de ~12s).
        if self._listo.wait(timeout=12.0):
            self._activo = True
            logger.info("Plugin discord_control listo (bot conectado).")
        else:
            self._activo = False
            logger.warning(
                "Discord: el bot no confirmó conexión en el tiempo de espera."
                " Último error: %s", self._ultimo_error or "(sin detalles)")
    ...
    async def _arrancar_cliente(self) -> None:
        ...
        # Necesarios para resolver miembros y leer contenido (configurados en
        # el portal; si no están activados, discord.py igual conecta pero
        # los miembros no aparecen).
    ...
    def _ejecutar_coro(self, coro: Any, timeout: float = 15.0) -> Any:
        ...
        Devuelve el resultado o lanza TimeoutError (el caller lo maneja).
        """
    ...
    def silenciar_usuario(self, usuario: str, silenciar: bool = True) -> str:
        ...
        try:
            ...
            return "Discord tardó demasiado en responder. Probá de nuevo."
        except Exception as e:  # noqa: BLE001
